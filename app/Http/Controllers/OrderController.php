...
class OrderController extends Controller
{
    public function AddOrder(Request $request)
    {
        ...
        $orders->rfirst_name = $request->input('first_name');        
        $orders->surname = $request->input('surname');
        $orders->rsurname = $request->input('surname');
        $orders->dob = $request->input('dob');
        $orders->rdob = $request->input('dob');
        $orders->document_number = $request->input('document_number');
        $orders->rdocument_number = $request->input('document_number');
        $orders->phone = $request->input('phone');
        $orders->rphone = $request->input('phone');
        $orders->email = $request->input('email');
        $orders->remail = $request->input('email');
        $orders->address = $request->input('address');
        $orders->raddress = $request->input('address');
        $orders->cap = $request->input('cap');
        $orders->rcap = $request->input('cap');
        $orders->country = $request->input('country');
        $orders->rcountry = $request->input('country');
        $orders->state = $request->input('state');
        $orders->rstate = $request->input('state');
        $orders->dist = $request->input('dist');
        $orders->rdist = $request->input('dist');
        $orders->city = $request->input('city');
        $orders->rcity = $request->input('city');
